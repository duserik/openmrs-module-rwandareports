...
public class SetupCancerScreeningLabReport {
    ...
    public void setup() throws Exception {
        ...
        ReportDesign createLabReportDesign = Helper.createRowPerPatientXlsOverviewReportDesign(createLabDefinition, "OncologyCancerScreeningLabreport.xls","OncologyCancerScreeningLabreport.xls_", null);
    ...
    private ReportDefinition createLabReportDefinition() {
       ...
       // reportDefinition.addParameter(new Parameter("location", "Health Center", Location.class));
        ...
        SqlCohortDefinition HPVLabRequestsWithoutLabResults=new SqlCohortDefinition("select distinct person_id from obs where concept_id=" +sampleMethod.getConceptId()+" and person_id not in (select distinct patient_id as person_id from encounter where form_id in ("+muzimaLabResultsform.getFormId()+","+openmrsLabResultsform.getFormId()+") and  encounter_datetime>= :endDate and voided=0) and obs_datetime<= :endDate and obs_datetime>= :startDate and voided=0");
        ...
        labReportSheetBaseCohort.getSearches().put(
                "2",
                new Mapped<CohortDefinition>(HPVLabResults, ParameterizableUtil.createParameterMappings("endDate=${endDate},startDate=${startDate}")));

        labReportSheetBaseCohort.getSearches().put(
                "3",
                new Mapped<CohortDefinition>(HPVLabRequestsWithoutLabResults, ParameterizableUtil.createParameterMappings("endDate=${endDate},startDate=${startDate}")));
...
//        dataSetDefinition.setSortCriteria(sortCriteria);
        ...
        dataSetDefinition.addColumn(RowPerPatientColumns.getMostRecentInperiodHavingCodedAnswers("HPVSample",sampleMethod,sampleMethodList,null,null,null),new HashMap<String, Object>());
